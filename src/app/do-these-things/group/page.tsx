...
export default async function Group() {
  ...
  const defaultGroup = await db.query.todos.findMany({
    where: (fields, {eq, and, isNull}) => and(isNull(fields.groupId), eq(fields.todoOwner, session.user.id)),
    columns: {
      todoChecked: true,
    },
  });
      ...
      <div className="flex min-h-0 w-full min-w-0 flex-col">
        <div className="flex w-full flex-wrap">
          ...
          <div className="flex w-full flex-grow flex-wrap gap-2 py-4">
            <Link href={"group/default"} className="flex h-fit items-center rounded-md bg-white">
