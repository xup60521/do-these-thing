...
export default function EditGroup({
  ...
  return (
    ...
    <AlertDialog open={openAlert} onOpenChange={onAlertClose}>
            <AlertDialogContent className={`${deleteGroupMutation.isPending && "cursor-wait"}`}>
                <AlertDialogHeader>
                    <AlertDialogTitle>Delete a group</AlertDialogTitle>
                    <AlertDialogDescription asChild>
                        <div className="flex flex-col gap-3">
                            <p>You can also delete all the relative to-dos, otherwise they will move to default group.</p>
                            <p className="flex gap-2 items-center">
                            <Checkbox id="check delete all relative to-dos" checked={deleteRelateTodos} onCheckedChange={()=>setDeleteRelateTodos(!deleteRelateTodos)} />
                            <Label htmlFor="check delete all relative to-dos">delete all relative to-dos?</Label>
                            </p>
                        </div>
                ...
                <AlertDialogFooter>
                    ...
                    <Button onClick={handleDeleteGroup} className={`${deleteGroupMutation.isPending && "cursor-wait"}`}>Sure!</Button>
                </AlertDialogFooter>
            </AlertDialogContent>
    </AlertDialog>
    </>
